...
/**
 * Get all service bookings for admin with filters
 */
export async function getAdminServiceBookings({ search = "", status = "" }) {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");

    // Verify db and serviceBooking model exist
    if (!db || !db.serviceBooking) {
      console.error("Database or serviceBooking model not available");
      throw new Error("Database not properly initialized. Please refresh the page.");
    }
    ...
    const bookings = await db.serviceBooking.findMany({
      ...
      orderBy: [{ serviceDate: "desc" }, { startTime: "asc" }],
    });
...
export async function updateServiceBookingStatus(bookingId, newStatus) {
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to update service booking status",
    };
  }
}
 ...
 */
export async function deleteServiceBooking(bookingId) {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");
