 ...
 *
 * Seeds the Car table with a few sample entries if it's empty so the /cars
 * page has something to work with in local/dev environments.
 */
export async function getCarFilters() {
  try {
    ...
    // Safe fallback so /cars page still renders even if DB/Prisma is misconfigured.
    return {
      success: false,
      data: {
        makes: [],
...
export async function getCars({
  ...
  try {
    // Get current user if authenticated. If Clerk isn't fully configured
    // (e.g. middleware not matching this route), fall back to guest mode
    // instead of throwing.
    let userId = null;
    try {
      ...
      console.warn("getCars auth error, continuing as guest:", authError);
    }
